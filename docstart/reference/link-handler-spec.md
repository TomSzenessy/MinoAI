# `/link` Handler Spec (Planned)

[Back to DocStart](../README.md)

Status:
- `planned`
- not implemented in the current frontend codebase
...
- `http://localhost:5173/link?serverUrl=...&apiKey=...`
...
5. Mark setup complete:
   ...
   - header: `X-Mino-Key: {apiKey}`
6. Persist linked server config in client storage/session.
7. Redirect to workspace/dashboard with success state.
...
- URL is cleaned after success (`apiKey` removed).
