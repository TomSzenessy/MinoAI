...
import { getFallbackProfile, type LinkedServerProfile } from '@/lib/storage';
...
export default function SettingsContent() {
	...
	const [agentStatus, setAgentStatus] = useState<AgentStatusPayload | null>(
		null
	);
	...
	const [channelForm, setChannelForm] =
		useState<ChannelFormState>(INITIAL_CHANNEL_FORM);
	...
	useEffect(() => {
		setChannelForm((previous) => {
			...
			const resolvedProvider =
				channelProviders.find(
					(entry) => entry.provider === previous.type
				) ?? fallbackProvider;
	...
	const activeChannelProvider = useMemo(
		() =>
			channelProviders.find(
				(providerEntry) => providerEntry.provider === channelForm.type
			) ??
			channelProviders[0] ??
			null,
	...
	const installedCatalogIds = new Set(
		serverPlugins.map((plugin) => plugin.id)
	);
					...
					<div className="rounded-mino-xl border border-[var(--glass-border)] bg-[var(--bg-elevated)] p-4">
						...
						{health ? (
							<p className="text-xs text-[var(--text-tertiary)]">
								{t('settings.server.lastVerified')}: v
								{health.version} · {health.uptimeSeconds}s
							</p>
						) : null}
						{healthError ? (
						...
						<div className="grid gap-2 sm:grid-cols-2">
							{(
								[
									{
										key: 'read',
										label: t('settings.agent.read')
									},
									{
										key: 'write',
										label: t('settings.agent.write')
									},
									{
										key: 'edit',
										label: t('settings.agent.edit')
									},
									{
										key: 'delete',
										label: t('settings.agent.delete')
									},
									{
										key: 'organize',
										label: t('settings.agent.organize')
									}
								] as const
							).map((permission) => (
								<label
					...
					<div className="grid gap-3 rounded-mino-xl border border-[var(--glass-border)] bg-[var(--bg-elevated)] p-4">
							...
							<select
								...
								onChange={(event) => {
									const nextType = event.target
										.value as ChannelProvider;
									const nextProvider =
										channelProviders.find(
											(entry) =>
												entry.provider === nextType
										) ??
										channelProviders[0] ??
										null;
									const credentialKeys =
										getCredentialKeys(nextProvider);
						...
						<label className="text-sm text-[var(--text-secondary)]">
							...
							<input
								...
								placeholder={t(
									'settings.channels.namePlaceholder'
								)}
							/>
						</label>
						...
						<label className="text-sm text-[var(--text-secondary)]">
							...
							<input
								...
								placeholder={t(
									'settings.channels.webhookSecretPlaceholder'
								)}
							/>
						</label>
						...
						{channelCredentialKeys.length === 0 ? (
							...
							<div className="grid gap-3">
								{channelCredentialKeys.map((key) => {
									const isRequired =
										activeChannelProvider?.requiredCredentials.includes(
											key
										) ?? false;
										...
										>
											...
											{isRequired ? (
												<span className="ml-1 text-[10px] uppercase tracking-widest text-[var(--text-tertiary)]">
													{t(
														'settings.channels.requiredField'
													)}
												</span>
											) : null}
											<input
												...
												placeholder={channelCredentialPlaceholder(
													key
												)}
											/>
										</label>
							...
							onClick={async () => {
								...
								const created = await upsertChannelOnServer(
									...
									profile.apiKey,
									{
										...
										webhookSecret:
											channelForm.webhookSecret.trim(),
										credentials:
											toChannelCredentials(channelForm)
									}
								);
									...
									<div className="flex items-center gap-2">
										...
										<button
											...
											onClick={async () => {
												...
												const updated =
													await updatePluginConfigOnServer(
														profile.serverUrl,
														profile.apiKey,
														plugin.id,
														{
